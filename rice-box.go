...
func init() {
	...
	file7 := &embedded.EmbeddedFile{
		...
		FileModTime: time.Unix(1527181040, 0),
	...
	fileb := &embedded.EmbeddedFile{
		...
		FileModTime: time.Unix(1527182406, 0),
	...
	filed := &embedded.EmbeddedFile{
		...
		FileModTime: time.Unix(1527183105, 0),
	...
	filei := &embedded.EmbeddedFile{
		Filename:    "crud/partials/structure.go.tmpl",
		FileModTime: time.Unix(1527181140, 0),
		Content:     string("\n// {{.Name}} {{.Description}}\ntype {{.Name}} struct {\n\tID        *{{pkeyPropertyType .PrimaryKey}} `json:\"id\"`\n\t{{- range .Fields}}\n\t{{.Name}} {{.Type}} `json:\"{{.Serialized}}\"`\n\t{{- end}}\n}\n\n// New returns an instance of {{.Name}}\nfunc New{{.Name}}() {{.Name}} {\n\treturn {{.Name}}{\n\t\tID: new({{pkeyPropertyType .PrimaryKey}}),\n\t\t{{- range .Initialization}}\n\t\t{{.Name}}: {{.Type}},\n\t\t{{- end}}\n\t}\n}\n\n// {{.Name}}Repository encapsulates operations that may be performed on the entity {{.Name}}\ntype {{.Name}}Repository struct {\n\tdb *sql.DB\n}\n"),
	}
	...
	dir4 := &embedded.EmbeddedDir{
		Filename:   "crud",
		DirModTime: time.Unix(1527178806, 0),
		ChildFiles: []*embedded.EmbeddedFile{
	...
	dir8 := &embedded.EmbeddedDir{
		...
		ChildFiles: []*embedded.EmbeddedFile{
			...
			filei, // "crud/partials/structure.go.tmpl"
			filej, // "crud/partials/update.go.tmpl"
	...
	embedded.RegisterEmbeddedBox(`templates`, &embedded.EmbeddedBox{
		...
		Files: map[string]*embedded.EmbeddedFile{
			...
			"crud/partials/structure.go.tmpl":          filei,
			"crud/partials/update.go.tmpl":             filej,
			"http.go.tmpl":                             filek,
			"rest.go.tmpl":                             filel,
