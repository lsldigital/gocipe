...
func generateMerge(entities map[string]util.Entity, entity util.Entity) (string, error) {
	var (
		before, sqlfieldsInsert, sqlfieldsUpdate, sqlPlaceholders, structFields []string
		count                                                                   = 1
	)
	...
	for _, ref := range entity.References {
		// IDField
		sqlPlaceholders = append(sqlPlaceholders, fmt.Sprintf("$%d", count))
		sqlfieldsUpdate = append(sqlfieldsUpdate, fmt.Sprintf(`"%s" = $%d`, ref.IDField.Schema.Field, count))
		sqlfieldsInsert = append(sqlfieldsInsert, fmt.Sprintf(`"%s"`, ref.IDField.Schema.Field))
		structFields = append(structFields, fmt.Sprintf("entity.%s", ref.IDField.Property.Name))
		count++

		// IDType
		sqlPlaceholders = append(sqlPlaceholders, fmt.Sprintf("$%d", count))
		sqlfieldsUpdate = append(sqlfieldsUpdate, fmt.Sprintf(`"%s" = $%d`, ref.TypeField.Schema.Field, count))
		sqlfieldsInsert = append(sqlfieldsInsert, fmt.Sprintf(`"%s"`, ref.TypeField.Schema.Field))
